...
function AIManager({ restartTutorial }) {
  const [showTutorial, setShowTutorial] = useState(() => {
    return localStorage.getItem("var-tutorial-completed") !== "true";
  });
  ...
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      text: "Hey! I'm Vidwath, your VAR Store Manager ⚽ Ask me anything about jerseys, boots, prices, or recommendations!",
    },
  ]);
  ...
  // ==========================================
  // RESTART TUTORIAL FROM TOUR BUTTON
  // ==========================================
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (!target) {
      console.log(`Tutorial target not found: ${targetId}`);
      return;
    }
  ...
  function finishTutorial() {
    localStorage.setItem("var-tutorial-completed", "true");
  ...
  async function handleSendMessage(customMessage = null) {
    const messageToSend = customMessage || userMessage.trim();
  ...
  const currentTargetId = tutorialSteps[step].target;
